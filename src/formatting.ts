...
import { type SupportedLanguage, registry } from "./i18n.js";
...
export function formatArrivals(
  language: SupportedLanguage,
  arrivals: LineArrival[],
): string {
  ...
  const linesText = arrivals.map((line) => {
    const arrivalTimesText = line.arrivalTimes
      ...
      .join(", ");
    return registry[language].templates.line(line.lineName, arrivalTimesText);
  });
  return linesText.join("\n");
}

export function formatFavoriteSaved(
  language: SupportedLanguage,
  alias: string,
  code: string,
): string {
  return registry[language].templates.favoriteSaved(alias, code);
}
